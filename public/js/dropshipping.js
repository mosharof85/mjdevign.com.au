...
let steps = Array.from(document.querySelectorAll('.stepsMain .stepActive'));
    document.querySelectorAll('.nextBtn a:not(.btn_checkout)').forEach(actionBtn=>{
        actionBtn.addEventListener('click', (e)=>{
            ...
            // validation.validate(currentStep);
            
            if(actionBtn.classList.contains('product-submit')){
                if(!(remainingCount === undefined || remainingCount === null) && (remainingCount > 0 )){
                    alert("You should select at most 200 products to proceed further! ( Until Products Remaining is 0/200)");
                    return;
                }
            }
